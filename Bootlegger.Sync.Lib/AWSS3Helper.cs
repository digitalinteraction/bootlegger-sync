...
namespace S3Downloader
{
    public class AWSS3Helper : IDisposable
    {
        ...
        private bool Open (bool throwOnError)
        {
            ...
            try
            {
                _client = new AmazonS3Client (_accessKeyID, _secretAccessKeyID, _awsRegion);
                return _client != null;
            }
            catch (Exception ex)
            {
        ...
        public event EventHandler<WriteObjectProgressArgs> OnProgress;
        ...
        public bool DownloadFile(string key, string destFullFilename, bool removeAfterDownload = false, bool throwOnError = false)
        {
            try
            {
                ...
                // download
                response.WriteObjectProgressEvent += (o, e) =>
                {
                    if (OnProgress != null)
                        OnProgress(o, e);
                };
